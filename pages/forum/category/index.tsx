...
const ForumCategories: NextPage = (data: any) => {
    ...
    useEffect(() => {
        async function getForums() {
            ...
            category.threadCount = threads.size
                    ...
                    </div>
                    {
                        forumsLoaded && (
                                <div className="flex flex-col px-6 py-6 rounded-xl">
                                    <h1 className="text-xl font-bold text-[#7F7B96]">{category.title}</h1>
                                    <div id="categories" className="flex flex-col gap-y-4">
                                        {
                                            forum.categories.map((thread) => (
                                                    <div className="dark:bg-primary gap-1 flex align-items-center" style={{ boxShadow: "0 0 0 1px rgb(255 255 255 / 10%)", borderRadius: "0.5rem", marginTop: "0.25rem", padding: "1rem 1.25rem"  }} key={thread.id + thread.title}>
                                                        ...
                                                        <div style={{ flexGrow: 1, lineHeight: 1.25, overflow: "hidden" }}>
                                                            ...
                                                            <h3 className="text-[#C8C7D8]" style={{fontSize: ".875rem", fontWeight: 400}}>by <Link href={"/user/" + thread.recentReply.profiles.username} className="font-bold"><h1>{thread.recentReply.profiles.full_name}</h1></Link>, {new Date(thread.recentReply.created_at || "1 January 1970").toLocaleTimeString("en-GB", { year: 'numeric', month: 'long', day: 'numeric' })}</h3>
                                                        </div>

                                                        <div className="text-[#C8C7D8] text-center" style={{ flexShrink: 0, width: "150px", fontSize: "0.85rem", fontWeight: 400 }}>
                                                            <span><strong>{thread.replies}</strong> Replies</span>
                                                        </div>
                                                                ...
                                                                <div className="flex align-items-center" style={{ flexShrink: 0, width: "300px", lineHeight: 1.25 }}>
                                                                    <div>
                                                                        <Link href={"/user/" + thread.recentReply.profiles.username}>
                                                                            <img referrerPolicy="no-referrer" className="rounded-full hover:rounded-lg" style={{width: "30px", height: "30px"}} alt="Avatar name" src={thread.recentReply.profiles.avatar_url} />
                                                                        </Link>
                                                                    </div>
